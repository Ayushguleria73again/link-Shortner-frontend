...
import { 
    Loader2, Cpu, Terminal, Shield, 
    Globe, Activity, Activity as Lock,
    ArrowRight 
} from 'lucide-react';
import RedirectionBackground from '@/components/redirection/RedirectionBackground';
import api from '@/lib/api';

export default function BrandedRedirectClient({ shortCode }) {
  ...
  useEffect(() => {
    const fetchLinkInfo = async () => {
      try {
        const { data } = await api.get(`/links/info/${shortCode}`);
  ...
  }, [loading, error, linkInfo, shortCode]);
